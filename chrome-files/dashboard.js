...
// Search toggle
searchBtn.onclick = () => { searchInput.style.display = searchInput.style.display==="none"?"block":"none"; searchInput.focus(); }
...
function deleteFolder(folderName){
  ...
  chrome.storage.sync.get(["blockedDomains"], d => {
    ...
    // Als je wilt dat domeinen niet verloren gaan, kan je ze naar Default verplaatsen in plaats van verwijderen
    chrome.storage.sync.set({blockedDomains: arr}, ()=>{
      // Reset selected folder naar Default
...
function render(){
  chrome.storage.sync.get(["enabled","blockedDomains","darkMode"], d=>{
    ...
    for(let folder in groups){
      ...
      // Voeg delete knop voor folder toe (behalve Default)
      if(folder !== "Default"){
        const delBtn = document.createElement("button");
        delBtn.textContent="🗑️";
        delBtn.style.marginLeft="8px";
        delBtn.onclick = (e)=>{
          e.stopPropagation(); // voorkomt dat click ook folder selecteert
          deleteFolder(folder);
        };
